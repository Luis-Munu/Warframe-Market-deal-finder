...
def scan_buys(name):
    ...
    if (item_list := utils.get_request(url)) is not None and len(list(item_list["buy_orders"])) != 0:
        item_list = list(item_list["buy_orders"])
        while True:
            for product in item_list:

                url = "items/" + product["item"]["url_name"] + "/orders?include=item"
                if (orders := utils.get_request(url)) is None:
                    continue
                orders = [
                    x for x in orders["orders"] if x["user"]["status"] == "ingame" and x["order_type"] == "sell" and
                    ("mod_rank" not in x or ("mod_rank" in x and x["mod_rank"] == product["mod_rank"]))
                ]
                if not orders:
                    continue
