...
def seller_prof(seller):
    ...
    if seller not in trades:
        seller_orders = utils.get_request("profile/" + seller + "/orders?include=profile")["sell_orders"]
        ...
        if item_num >= 6:
            if len(trader["items"]) > 1:
                ...
                trades[seller]["value"] = trades[seller]["ducats"] / trades[seller]["price"]
...
def get_orders(limit):
    ...
    for line in ducat_list.readlines():
        ...
        result = utils.get_request("items/" + item_name + "/orders")["orders"]
        result = [
            x for x in result
            if x["user"]["status"] == "ingame" and x["user"]["reputation"] > 1 and x["order_type"] == "sell"
        ]
        if len(result) <= 1:
            continue
        ...
        if item_data["value"] >= limit:
            line = (datetime.now().strftime("%H:%M:%S") + " item: " + item_name.title().replace("_", " ") +
                    " ducats/plat: " + str(item_data["value"]) + " price: " + str(item_data["price"]) + " quantity: " +
                    str(item_data["quantity"]) + " seller: " + item_data["seller"])
            print(line)
            ...
            if len(trades) >= 6:
                ...
                for i in range(1, 7):
                    line = "Seller: " + str(list(trades)[-i]) + " ducats/plat: " + str(
                        trades[list(trades)[-i]]["value"]) + " price: " + str(
                            trades[list(trades)[-i]]["price"]) + " ducats: " + str(
                                trades[list(trades)[-i]]["ducats"]) + "\n"
                    print(line)
            ...
            for set_item in result:
                if set_item["url_name"] not in string_set:
                    if set_item["ducats"] >= 45:
                        ducat_list.write(set_item["url_name"] + "," + str(set_item["ducats"]) + "\n")
