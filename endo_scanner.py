...
def get_mods():
    ...
    with open("mods.json", encoding="utf8") as json_file:
        ...
        data = [x for x in data if "fusionLimit" in x and x["fusionLimit"] == 10 and x["rarity"] != "common"]

        return [{"name": x["name"], "max_rank": x["fusionLimit"], "rarity": x["rarity"]} for x in data]
    ...
    while True:
        for item in item_list:
            ...
            if result:
                ...
                result = [
                    x for x in result if x["order_type"] == "sell" and x["platform"] == "pc" and
                    x["mod_rank"] == item["max_rank"] and x["user"]["status"] == "ingame"
                ]
                if not result:
                    continue
                order = sorted(result, key=lambda x: x["platinum"])[0]
                value = (rarity_value[item["rarity"]] / order["platinum"] if "rarity" in item else item["value"] /
                         order["platinum"])
                if value > endo_limit:
                    print(datetime.now().strftime("%H:%M:%S") + " item: " + item["name"] + " endo/plat: " + str(value) +
                          " price: " + str(order["platinum"]) + " quantity: " + str(order["quantity"]) + " seller: " +
