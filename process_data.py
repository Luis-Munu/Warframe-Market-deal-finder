...
def process_data(response):
    ...
    for auction in response:
        ...
        if settings.scan_riven is True:
            if (
                auction["buyout_price"]
                and auction["item"]["weapon_url_name"] in settings.wished_rivens
            ):
                stat_list = [stat[1] for stat in stats]
                for wished_roll in settings.wished_rivens[auction["item"]["weapon_url_name"]]:
                    list_diff = utils.difference_lists(
                        stat_list, list(wished_roll["stats"].values())
                    )
                    if len(list_diff) == 1 and stats[-1][0] is False:
                        if auction["buyout_price"] <= wished_roll["price"]:
                            print(
                                "found a wished roll! "
