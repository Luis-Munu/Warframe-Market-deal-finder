    ...
    for auction in utils.get_current_auctions():
        if relist_without_bids and not auction["top_bid"] or not relist_without_bids:
            ...
            if result.status_code == 200:
                print(auction["item"]["weapon_url_name"].capitalize().replace("_", " ") + " " +
                      auction["item"]["name"].capitalize().replace("_", " ") + " succesfully cancelled")
            else:
                print(auction["item"]["weapon_url_name"].capitalize().replace("_", " ") + " " +
                      auction["item"]["name"].capitalize().replace("_", " ") + " not cancelled")
            # relists them.
            ...
            if result.status_code == 200:
                print(auction["item"]["weapon_url_name"].capitalize().replace("_", " ") + " " +
                      auction["item"]["name"].capitalize().replace("_", " ") + " succesfully posted")
            else:
                print(auction["item"]["weapon_url_name"].capitalize().replace("_", " ") + " " +
                      auction["item"]["name"].capitalize().replace("_", " ") + " not posted")
