...
import src.riven_stuff.riven as riven
...
def process_data(response):
    ...
    for auction in response:
        if auction["closed"] is True or "ingame_name" not in auction["owner"]:
            continue
        if (
        ...
        name = auction["item"]["weapon_url_name"].replace("_", " ").title()
        if len([i for i in settings.weapons if i["name"] == name]) == 0:
            continue
...
def export_txt(results):
    ...
    each riven has its own different paths depending on the weapon and the rating."""
