...
def deal_finder():
    ...
    item_list = utils.get_wfm_request("items")
    if not item_list:
        return
    item_list = [x["url_name"] for x in item_list["items"]]
    while True:
        for item in item_list:
            item_stats = utils.get_wfm_request("items/" + item + "/statistics")
            ...
            item_orders = utils.get_wfm_request(
                "items/" + item + "/orders?include=item"
            )
            if item_orders is None:
                continue

            item_orders = [
                x for x in item_orders["orders"] if x["user"]["status"] == "ingame"
            ]
                ...
                for rank_stats in stat_list:
                    sell_rank = (
                        [
                            ...
                            if "mod_rank" in x
                            and x["mod_rank"] == rank_stats["mod_rank"]
                        ]
                        if "mod_rank" in rank_stats
                        else sell_orders
                    )
                    ...
                    if not i:
                        rank_stat = (
                            next(
                                x
                                for x in rank_stats
                                if order["mod_rank"] == x["mod_rank"]
                            )
                            if isinstance(type(rank_stats), list)
                            else rank_stats
                        )

                    if (
                        comparator(order, [rank_stat, sell_rank][i], i)
                        and order["platinum"] > 10
                        and (rank_stats["volume"] > 5 or order["platinum"] > 100)
                    ):
                        print_deal(
                            item,
        ...
        mod_stats.append(
            {
                "name": item,
                "mod_rank": key,
                "item_2d_average": sum([tmp[0] for tmp in rank_stats])
                / len(rank_stats),
                "lowest_2d_average": sum([tmp[1] for tmp in rank_stats])
                / len(rank_stats),
...
def analyze_part(item, item_stats):
    ...
    return {
        "name": item,
        "item_2d_average": sum([timestamp["avg_price"] for timestamp in item_stats])
        / len(item_stats),
        "lowest_2d_average": sum([timestamp["min_price"] for timestamp in item_stats])
        / len(item_stats),
...
def comparator(order, item_stats, mode):
    ...
    if mode:
        return order["platinum"] < (
            sum([c["platinum"] for c in item_stats[0:3]]) / 3 * 0.7
        )
