...
def scan_relics():
    ...
    with open("best_sell_relics.csv", "w+") as sell_relic:
        with open("best_buy_relics.csv", "w+") as buy_relic:
            ...
            relic_list = [
                relic for relic in relic_list if relic["state"] in ("Intact", "Radiant")
            ]
            print(
                "Analyzing "
                + str(len(relic_list))
                + " relics and "
                + str(len(relic_list) * 5)
                + " items."
            )
            for relic in relic_list:
                relic_stats = utils.get_wfm_request(
                    "items/"
                    + relic["tier"].lower()
                    + "_"
                ...
                relic_orders = utils.get_wfm_request(
                    "items/"
                    + relic["tier"].lower()
                    + "_"
                    + relic["relicName"].replace(" ", "_").lower()
...
def analyzeRelic(relic, relic_orders):
    ...
    analyzed_relic = {}
    analyzed_relic["name"] = (
        relic["tier"] + " " + relic["relicName"] + " " + relic["state"]
    )
    ...
    for reward in relic["rewards"]:
        ...
        if reward["itemName"] not in item_dict:
            reward_data = utils.get_wfm_request(
            ...
            reward_values.append(
                item_dict[reward["itemName"]] * reward["chance"] * 0.01
            )
