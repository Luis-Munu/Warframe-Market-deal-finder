...
def choose_category():
    ...
    if cat == 1:
        ...
        import src.weapon_updater

        settings.load_data()
        src.weapon_updater.update_weapons()
...
def choose_scan_mode():
    ...
    while True:
        ...
        if 0 < mode < 9:
            break
    if mode == 1:
        from src.ducanator import use_ducanator

        use_ducanator(ask_for_ducat_scan())
    elif mode == 2:
        scan_for_deals()
    elif mode == 3:
        import src.scanners.part_scanner as part_scanner

        part_scanner.part_scanner()
    elif mode == 4:
        endo_scan()
    elif mode == 5:
        import src.scanners.relic_scanner as relic_scanner
        ...
        email, password = ask_for_email(), ask_for_pass()
        part_scanner.upvote_farmer(email, password)
...
def ask_for_email():
    ...
    while True:
        try:
            email = str(
                input(
                    "Introduce the email associated to your warframe.market account: "
                )
            )
        except ValueError:
...
def ask_for_name():
    ...
    while True:
        try:
            ...
            print(
                "An error happened when reading your profile name, please try again. \n"
            )
        if name:
            return name
...
def relist_auctions():
    ...
    import src.riven_stuff.riven_relisting

    while True:
        print(
            "Do you want to relist all your auctions or only the ones without bids?:\n"
        )
    ...
    src.riven_stuff.riven_relisting.relist_auctions(relist_type)
...
def define_search():
    ...
    while True:
        ...
        print(
            "2. Slow search (high amount of rivens. will also search for godrolls for the wanted weapons)\n"
        )
        print(
            "3. Specific search (only searches for a weapon or your specific combinations)\n"
        )
        try:
...
def weapon_combo():
    ...
    while True:
        ...
        if (
            weapon_name in settings.weapons[utils.get_weapon_type(weapon_name)].keys()
...
def ask_for_stats():
    ...
    for i in range(4):
        ...
        if statnum < len(stat_list):
            settings.weapon_stats[stat_name[i]] = (
                stat_list[statnum].lower().replace(" ", "_")
            )
...
def ask_for_ducat_scan():
    ...
    print(
        "The program will scan the market for items with a ducat value higher than your limit.\n"
    )
    print(
        "At the end of the scan, it'll provide you with a list of the best sellers to buy off the market.\n"
    )
    while True:
...
def riv_search():
    ...
    import src.request_processor
    import src.riven_stuff.riven_requests
    import src.url_creation

    # gets the urls for the searches we have to do.
    res = src.url_creation.data_creation()
    # requests said searches to the server and returns them.
    res = src.riven_stuff.riven_requests.mass_riven_request(res)
    # processes the requests and converts them into riven objects.
    res = src.request_processor.process_data(res).values()
    # sorts them by price.
    res = sorted(res, key=lambda riven: (riven.buyout_price, riven.real_value))
    # exports them into txt files.
    src.request_processor.export_txt(res)
...
def scan_for_deals():
    ...
    import src.scanners.deal_scanner as deal_scanner
...
def scan_buys():
    ...
    import src.scanners.buy_scanner as buy_scanner
...
def endo_scan():
    ...
    import src.scanners.endo_scanner as endo_scanner
