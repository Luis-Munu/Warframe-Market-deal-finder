...
#Used for specific rolls. Given a weapon, it creates a url based on its stats, if no stats are given we create
#a list of urls with generic godrolls. If unroll boolean is True, we return an unrolled url
def createSpecificUrl(weapon, stats = [], unroll = False):
    ...
    if stats:
        if "stat3" in stats: 
            ...
            return [url + neg for url in addPositive(settings.WFM_API + "/auctions/search?type=riven&weapon_url_name=" 
            + weapon + "&positive_stats=" + stats["stat1"] + "," + stats["stat2"] , getWeaponType(weapon))]
    else:
...
def addStats(weapon):
    weaponTypes = getWeaponType(weapon)
    res = set()
    for weaponType in weaponTypes:
        for combination in combinations(settings.decentPositives[weaponType], 3):
