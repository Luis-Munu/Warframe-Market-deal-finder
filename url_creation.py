...
def create_specific_url(weapon, stats=[], unroll=False):
    ...
    if stats:
        ...
        neg = (
            "&negative_stats=" + stats["negative"] if "negative" in stats else "&negative_stats=has"
        )
        return [
            url + neg
...
def add_stats(weapon):
    ...
    weapon_types = get_weapon_type(weapon)
    res = set()
...
def unrolled_urls():
    ...
    result = set()
    unroll = (
        [k for d in settings.weapon_list for k in d]
        if settings.non_existant_search or settings.slow_search
        else settings.wished_unrolleds
    )
...
def data_creation():
    ...
    if (
        settings.specific_search
        or settings.fast_search
        or settings.medium_search
        or settings.slow_search
        or settings.non_existant_search
    ):
        url_list.extend(specific_urls())
    # creates the url of a specific weapon search
    if settings.specific_weapon:
        url_list.extend(weapon_urls(settings.weaponName, settings.weapon_stats))

    url_list = list(set(url_list))
    time_left = time.strftime("%H:%M:%S", time.gmtime(len(url_list)))
    print(
        str(len(url_list)) + " possible riven combinations. estimated wait time: " + str(time_left)
    )
