from ducanator import useDucanator
import settings, rivenRelisting, utils
...
def weaponCombo():
    while True:
        ...
        if weaponName in settings.weaponList and utils.checkVariant(weaponName) == False: 
...
def scanRiven():
    while True:
        ...
        try: scan = int(input())
            ...
            print("Introduce a number between 1 and 2")
        if scan == 1:
            settings.scanRiven = True
            break
        elif scan == 2 : break

def askForDucatScan():
    while True:
        print("Would you like the program to keep searching for good ducat/plat items constantly?\n")
        print("If the program finds an item with a good ducat/plat ratio it will save it and show a message.\n")
        print("1. Yes\n")
        print("2. No\n")
        try: scan = int(input())
        except ValueError:
