...
def scale_range(actual, actual_min, actual_max, wanted_min, wanted_max):
    ...
    return (wanted_max - wanted_min) * ((actual - actual_min) / (actual_max - actual_min)) + wanted_min
...
def wfm_string(item_name, seller_name, price, rank=None):
    ...
    if rank is not None:
        print("/w " + str(seller_name) + " hi! i want to buy: " + item_name.title().replace("_", " ") + " (rank " +
              str(rank) + ") for " + str(price) + " platinum. (warframe.market)\n")
    else:
        print("/w " + str(seller_name) + " hi! i want to buy: " + item_name.title().replace("_", " ") + " for " +
              str(price) + " platinum. (warframe.market)\n")
